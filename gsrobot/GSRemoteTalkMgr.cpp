...
void *APlayThread(LPVOID lpPara)
{
	...
	pthread_detach(pthread_self()); //jyc20170722 debug
...
void GSRemoteTalkMgr::OnProc( const int Play_id )
{
    ...
    if(!RTMP_Connect(rtmp,NULL)){
        RTMP_Free(rtmp);
	printf("DEBUG Connect err.......\n");
	return;
    }
    if(!RTMP_ConnectStream(rtmp,0)){
        RTMP_Close(rtmp);
        RTMP_Free(rtmp);
	printf("DEBUG ConnectStream err.......\n");
	return;
    }
    int nRead,buflen=0;
...
void GSRemoteTalkMgr::UrlKey_Check( bool forceCheck )
{
	...
	// ÿ������һ��
	if( !forceCheck && (curtime-m_last_checktime) < 5000 )
	{
		return;
	}
	...
	for( ; it != m_mapTalk.end(); ++it )
	{
		if( it->second.isOnlyOneTalk && it->second.play )
		{
			// ����һ��ֹͣ����
			bool isFoundPlay = false;
			it = m_mapTalk.begin();
